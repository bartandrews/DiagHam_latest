  ...
  for (int n=0; n<=NMax; ++n)
    {
      ...
      if (fabs(rstAP - rstDouble)>1e-10)
	    cout << "For x="<<xDouble<<", the discrepancy between Double and AP at n="<<n<<" is "<< rstAP - rstDouble<<", AP="<<rstAP<<", Double="<< rstDouble<<",AP2="<<mpf_get_d(rsts[n])<<endl;

      mpf_reldiff(TmpValue, rst, rsts[n]);
      mpf_set_d(TmpValue2,0.0);
      // if (mpf_cmp (TmpValue, TmpValue2))
      if (mpf_cmp (rst, rsts[n]))
	    cout << "For x="<<xDouble<<", n="<<n<<", have a relative discrepancy between AP1 and AP2 codes of "<<mpf_get_d(TmpValue)<<endl;
    }
}
...
void LaguerrePolynomialRecursionAP::EvaluateLaguerrePolynomials(int N, mpf_t* rst, mpf_t &x)
{
  ...
  for (int n=2; n<N; ++n)  // do not change this from 'n<N' to 'n<=N'
    {
      mpf_set_d (TmpValue,2.*n-1.);
      mpf_sub (TmpValue2, TmpValue, x);
