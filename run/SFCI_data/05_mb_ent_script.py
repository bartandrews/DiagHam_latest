...
if __name__ == "__main__":
    ...
    for alpha in [1]:
        ener_path = f"/home/bart/DiagHam_latest/run/SFCI_data/{stats}_alpha_{alpha}/q_{q:g}"
        os.chdir(ener_path)
        ...
        for t3hop in np.linspace(-0.25, 0, 26):
            ...
            for file in os.listdir("."):
                if f"t3_{t3hop:.2f}_" in file and file.endswith("_0_ext.dat"):
                    given_ext_file = file
            ...
            for i in range(s):
                ...
                print(f"t3 = {t3hop:.2f} ==> ({kx_min[i]}, {ky_min[i]}): {E_min[i]}")
            ...
            taskfile_name = f"taskfile_t3_{t3hop:.2f}"
            ...
            for i in range(s):
                ...
                if stats == "fermions":
                    command = f"{FCIHofstadterModel} -p {p} -X {X} -Y {Y} -x {x} -y {y} --t3hop {t3hop:.2f} -m 32000 " \
                    ...
                    command = f"{FCIHofstadterModel} --boson -p {p} -X {X} -Y {Y} -x {x} -y {y} --t3hop {t3hop:.2f} -m 32000 " \
                              f"-S --processors 6 -n 5 --lanczos-precision 1e-10 " \
