...
def band_geom(t3, t6, t9, q, grain, grain_r):
    ...
    return [t3, t6, t9, tism_int, dism_int, berry_fluc, fs_fluc, gap_width]
...
if __name__ == "__main__":
    ...
    results = np.array(Parallel(n_jobs=6)(delayed(band_geom)(t3, 0, 0, q_val, grain_val, grain_r_val) for t3 in ts))

    file = open(f"q_{q_val}.txt", "w")
    for i in range(np.shape(results)[0]):
        file.write(f"{results[:, 0][i]:.2f}\t{results[:, 1][i]:.2f}\t{results[:, 2][i]:.2f}\t"
                   f"{results[:, 3][i]}\t{results[:, 4][i]}\t{results[:, 5][i]}\t"
                   f"{results[:, 6][i]}\t{results[:, 7][i]}\n")
    file.close()
