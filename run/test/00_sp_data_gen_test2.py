...
if __name__ == "__main__":
    ...
    ts = np.linspace(-0.25, 0.25, 51)

    results = np.array(Parallel(n_jobs=6)(delayed(band_geom)(t6, t6, q_val, grain_val, grain_r_val) for t6 in ts))

    file = open(f"q_{q_val}_2.txt", "w")
    for i in range(np.shape(results)[0]):
        file.write(f"{results[:, 0][i]:.2f}\t{results[:, 1][i]:.2f}\t{results[:, 2][i]}\n")
    file.close()
